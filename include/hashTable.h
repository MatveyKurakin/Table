 ...
 public:
  ...
  hashIterator& operator++() {
    ...
    if (index2 < (it_table->storage)[index1].size() - 1)
      ...
      while ((it_table->storage)[index1].size() == 0) index1 = (index1 + 1) % (it_table->storage).size();
      index2 = 0;
    }
  ...
  bool operator==(const hashIterator& other) { return iterator == other.iterator; }
  bool operator!=(const hashIterator& other) { return iterator != other.iterator; }
};
...
class HashTable {
 protected:
  ...
  double calculateAverageCollisions() {
    ...
    for (int i = 0; i < size(); i++) {
      if (storage[i].size()) ans += (double)(storage[i].size() - 1);
    }
    return ans;
  }
  ...
  bool check() {
    ...
    if ((averageCollisions >= ((double)sizeoftable() / 2)) || (percentageFilling > 150)) return true;
    return false;
  }
  void rebalancing(int offset) {
    ...
    for (int i = 0; i < 10; i++) {
      if (i == 9) {
        if (primeNumbers[i] == last_p)
          p = primeNumbers[0];
        else if (primeNumbers[i] == last_q)
          q = primeNumbers[0];
      } else {
        if (primeNumbers[i] == last_p)
          p = primeNumbers[i + 1];
 ...
 public:
  ...
  int HashFunction(std::vector<int> key) {  
    int pos = 0;
    int p_step = 2;
    int hashP = p_step;
    for (int i = 0; i < key.size(); i++) {
  ...
  int HashFunction(float key) {  
    ...
    size_t result = 0;
    for (int i = 0; i < 64; i++) {
      if (((size_t)mask & (*a)) != 0) result += mask;
      mask << 1;
    }
  ...
  HashTable() { storage = vector<vector<pair<TypeKey, TypeData>>>(); }
  HashTable(const HashTable& other) { storage = other.storage; }
  ~HashTable() { storage = vector<vector<pair<TypeKey, TypeData>>>(0); }
  HashTable(int size) { storage = vector<vector<pair<TypeKey, TypeData>>>(size); }
  ...
  hashIterator<TypeKey, TypeData> begin() {
    ...
    int index = 0;
    while (storage[index].size() == 0) index++;
